...
VK::VK(std::istream* fs, unsigned int offset) : m_offset(offset), m_fs(fs), retrieved_data(0), m_resident(false)
{
	...
	Helper::Read(m_fs, 0x1000 + m_offset + 0x06, sizeof(int), &m_name_length);
	Helper::Read(m_fs, 0x1000 + m_offset + 0x08, sizeof(int), &m_data_length);
	Helper::Read(m_fs, 0x1000 + m_offset + 0x0C, sizeof(int), &m_data);
	Helper::Read(m_fs, 0x1000 + m_offset + 0x10, sizeof(int), &m_type);

	m_name = new char[(long long)m_name_length + 1];
	Helper::Read(m_fs, 0x1000 + m_offset + 0x18, sizeof(int), &m_name);
